...
FireFirstArrivalRasterUpdate
FireFirstArrivalRaster::update_from_front_rk2_sweep (
    const FireFront& start_front,
    ...
    std::size_t temporal_substeps)
{
    ...
    if (temporal_substeps == 0) {
        throw std::invalid_argument(
            "Fire first-arrival front RK2 sweep temporal_substeps must be positive");
    }
    if (!std::isfinite(start_time_s)
        || !std::isfinite(end_time_s)
        || !std::isfinite(time_tolerance_s)) {
    ...
    try {
        ...
        local_error =
            "unknown local Fire first-arrival front RK2 sweep error";
    }
    ...
    if (failed != 0) {
        if (!local_error.empty()) {
            throw std::runtime_error(
                "distributed Fire first-arrival front RK2 sweep failed: "
                + local_error);
        }

        throw std::runtime_error(
            "distributed Fire first-arrival front RK2 sweep failed on another MPI rank");
    }
